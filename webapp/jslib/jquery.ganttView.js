			...
			for (var i = 0; i < 12; i++) {
				if (months[i]) { 
					for (var j = 0; j < months[i].length; j++) {
						...
						if (DateUtils.isWeekend(months[i][j]) && showWeekends) { 
			...
			for (var i = 0; i < data.length; i++) {
				for (var j = 0; j < data[i].series.length; j++) {
					...
					var blockDiv = jQuery("<div>", { 
						...
						"title": data[i].series[j].startReal+"至"+data[i].series[j].endReal,
						"css": { 
...
//					blockDiv.append($("<div>", { "class": "ganttview-block-text" }).text(data[i].series[j].name));
